    ...
    if df is not None:
        ...
        if len(y_cols) > 0:
            ...
            ALIAS_MAP = {
                ...
                "TopDrive.TRQ.1.TRC": "TRQ",
                "TopDrive.RPM.1.TRC": "RPM",
                "DrillBoreHoleReaming.WOBDriller": "WOB",
                "DrillBoreHoleReaming.ROPOnDepthStep": "ROP",
                "FlowLineSystem.Fpdl.1.TRC": "Flow Out",
                "DegasserOut_1.MW.Modbus.TRC": "MW_Out",
                "DegasserIn_1.MW.Modbus.TRC": "MW_In",
                "DegasserOut_1.TMP.Modbus.TRC": "Tmp_out",
                "DegasserIn_1.TMP.Modbus.TRC": "Tmp_In"
            }
            
            # IDEAL DEFAULT TEMPLATES
            ideal_t1 = [
            ...
            # --- TRI-TRACK SELECTION UI ---
            t1_col, t2_col, t3_col = st.columns([2, 1, 1])
            with t1_col:
                track1_params = st.multiselect("Select Parameters for TRACK 1", base_params, default=def_t1, format_func=lambda x: ALIAS_MAP.get(x, x))
            with t2_col:
                track2_params = st.multiselect("Select Parameters for TRACK 2", base_params, default=def_t2, format_func=lambda x: ALIAS_MAP.get(x, x))
            with t3_col:
                track3_params = st.multiselect("Select Parameters for TRACK 3", base_params, default=def_t3, format_func=lambda x: ALIAS_MAP.get(x, x))
                    ...
                    for track_col, t_params, t_name in track_configs:
                        with track_col:
                            ...
                            for param in t_params:
                                ...
                                short_name = ALIAS_MAP.get(param, param)
                                ...
                                with c_name:
                                    st.markdown(f"<h5 style='color:{color}; margin-top:28px; font-size:14px;'>{short_name}</h5>", unsafe_allow_html=True)
                                with c_min:
                    ...
                    with st.spinner("Processing tracks and syncing axes..."):
                        ...
                        for param in all_selected_params:
                            ...
                            if x_col in df.columns and y_col in df.columns:
                                ...
                                # 🚨 UPDATED: DYNAMIC SQUARE-DROP GAP FILLER (TRQ and WOB included) 🚨
                                short_name = ALIAS_MAP.get(param, param)
                                if short_name in ["ROP", "TRQ", "WOB"]:
                                    time_diffs = temp_df['Time'].diff()
                                    
                                    # Set the threshold based on the parameter
                                    if short_name == "ROP":
                                        threshold = pd.Timedelta(minutes=10)
                                    elif short_name in ["TRQ", "WOB"]:
                                        threshold = pd.Timedelta(minutes=5)
                                        
                                    large_gaps = time_diffs > threshold
                                    
                                    if large_gaps.any():
                                        # Inject a '0' reading 1 second after operation stops
                                        gap_starts = temp_df[large_gaps].copy()
                                        gap_starts['Time'] = gap_starts['Time'] - time_diffs[large_gaps] + pd.Timedelta(seconds=1)
                                        gap_starts[y_col] = 0.0
                                        
                                        # Inject a '0' reading 1 second before operation resumes
                                        gap_ends = temp_df[large_gaps].copy()
                                        gap_ends['Time'] = gap_ends['Time'] - pd.Timedelta(seconds=1)
                                        gap_ends[y_col] = 0.0
                                        
                                        # Splice the new zeros into the timeline
                                        temp_df = pd.concat([temp_df, gap_starts, gap_ends]).sort_values('Time').reset_index(drop=True)
                        ...
                        if not filtered_traces:
                                ...
                                for i, param in enumerate(t_params):
                                    ...
                                    if i == 0:
                                        layout_updates[layout_axis_name] = dict(
                                            title=dict(text=short_name, font=dict(color=color, size=12), standoff=0), 
                            ...
                            export_dfs = []
                            for param, f_df in filtered_traces.items():
                                short_name = ALIAS_MAP.get(param, param)
                                ...
                                # Group by the rounded Time and take the average
                                temp = temp.groupby('Time').mean()
                                
                                export_dfs.append(temp)
                                
                            if export_dfs:
                                # Mathematically stitch all dataframes together based on exact time
                                export_df = pd.concat(export_dfs, axis=1)
                                export_df = export_df.sort_index().reset_index()
                                
                                # FORCE EXACT TIME FORMAT
                                export_df['Time'] = export_df['Time'].dt.strftime('%m/%d/%Y %H:%M:%S')
                                
                                # FORCE ALL SELECTED COLUMNS TO APPEAR (EVEN IF EMPTY)
                                for param in all_selected_params:
                                    short_name = ALIAS_MAP.get(param, param)
                                    if short_name not in export_df.columns:
                                        export_df[short_name] = None
                                        
                                # Reorder the columns to match Track 1, 2, and 3 exactly
                                final_cols = ['Time'] + [ALIAS_MAP.get(p, p) for p in all_selected_params]
                                ...
                                st.download_button(
                                    label="Download Selected Parameters as CSV",
                                    data=csv_data,
                                    file_name=f"RigData_{start_date.strftime('%Y%m%d')}_{start_time.strftime('%H%M')}.csv",
                                    mime="text/csv"
                                )
            else:
